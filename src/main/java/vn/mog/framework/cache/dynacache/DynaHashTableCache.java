  ...
  public synchronized Object getCachedItem(Object key, DynaCacheItemFeed _feeder) {
    ...
    if (items.containsKey(key)) {// in cache
      ...
      if (items.size() > cacheSize) {// capacity exceeded
        ...
        /*
         * Since the capacity had exceeded we need to remove some items to get more space Here, for
         * simplicity we remove the eldest item, however there exist some others ways to deal with
         * this Some possible solutions are to remove the least accessed items, items exceeded time
         * to live, ...
         */
