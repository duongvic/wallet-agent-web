...
public class PassiveRedisMemoryCache implements PassiveDynaCache {
    ...
    if (value != null) {
      try {
        CachedItem cachedItem =
            new CachedItem(value, timeToLive * 1000, Calendar.getInstance().getTime().getTime());
        redisTemplate.opsForValue().set(key, cachedItem, Long.valueOf(timeToLive + ""),
            TimeUnit.SECONDS);
      } catch (Exception e) {
        e.printStackTrace();
      }
  ...
  public void updateCachedItem(Object key, Object value) {
    try {
      ...
      try {
        timeToLive =
            (int) ((cached.getTimeToLive() - (Calendar.getInstance().getTime().getTime() - cached
