...
<div class="page">
  <div class="page-header">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active"><a href="#"><spring:message code="label.epin"/></a></li>
    </ol>
      ...
      <div class="col-md-7">
        <div class="panel panel-bordered">
          <div class="panel-body py-10">
            <form class="form-horizontal" method="post" action="/pin-code/pin-pay-verify">
              ...
              <div class="row">
                <c:forEach var="enabledService" items="${enabledServices}">
                  <c:choose>
                    <c:when test="${'040101' eq enabledService.serviceCode}">
                      <c:set var="serviceName" value="Viettel"/>
                    </c:when>
                    <c:when test="${'040102' eq enabledService.serviceCode}">
                      <c:set var="serviceName" value="Vinaphone"/>
                    </c:when>
                    <c:when test="${'040103' eq enabledService.serviceCode}">
                      <c:set var="serviceName" value="MobiFone"/>
                    </c:when>
                    <c:when test="${'040104' eq enabledService.serviceCode}">
                      <c:set var="serviceName" value="GMobile"/>
                    </c:when>
                    <c:when test="${'040105' eq enabledService.serviceCode}">
...
<script>
  ...
  $(document).ready(function () {
    ...
    loadCardCommission("${tenSP != null ? tenSP : 'Viettel'}", $("#faceValue"), '',
    ...
    $(".phoneCharge-choose label").click(function (e) {
      ...
      loadCardCommission(alt, $("#faceValue"), '', $("#faceValue").val(), "${_csrf.parameterName}",
        "${_csrf.token}");
    });
  ...
  function thanhToan() {
    ...
    $('#disCount').html(formatCurrency($('#faceValue').val() * $('#quantity').val() * currentCommission[$('#faceValue').val()].commision / 100).concat(' ').concat('Zpoint'));
  ...
  function onLoadCommissionToModel(commissionType, data, commissionFee, prices) {
    ...
    $('#disCount').html(
      formatCurrency(parseInt(parseInt($('#faceValue').val() * $('#quantity').val()) * currentCommission[prices[0]].commision / 100 + currentCommission[prices[0]].commisionFixedAmount)));
    thanhToan();
