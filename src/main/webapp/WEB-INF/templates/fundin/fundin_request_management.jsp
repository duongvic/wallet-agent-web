...
<c:if test="${'cash-on-hand' eq fund_in_method}">
    <spring:message code="fundin.cash.on.hand" var="fund_in_method_label"/>
</c:if>
    ...
    <div class="page-header">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#"><spring:message
                    code="label.request.fundin.points"/> </a></li>
        </ol>
                    ...
                    <div class="col-sm-12 col-md-4 col-lg-4 text-right search_entry mb-10">
                        ...
                        <c:if test="${'cash-on-hand' eq fund_in_method}">
                            <button type="button" class="btn btn-primary btn-sm mb-xs mt-xs"
                                    onclick="window.location.replace('/fundin/${fund_in_method}/create')">
                        ...
                        <c:if test="${'bank-transfer' eq fund_in_method}">
                            <button type="button" class="btn btn-primary btn-sm mb-xs mt-xs"
                                    onclick="window.location.replace('/fundin/${fund_in_method}/create/info')">
                                <i class="fa fa-plus"></i>&nbsp;<spring:message
                                    code="fundorder.request.create.label"/>
                            </button>
                        </c:if>
            ...
            <div class="panel-body tb-tool">
                <table
                    ...
                    <tr>
                        ...
                        <th><spring:message
                                code="fundorder.table.header.label.syntax"/></th>
                        <th><spring:message
                        ...
                        <th><spring:message
                                code="fundorder.request.detail.remark.label"/></th>
                        <th><spring:message
...
<script type="text/javascript">
  ...
  function drawTableList() {
    ...
    var serviceTypeId = ['FUND_IN'];
    var orderChanelId = [('cash-on-hand' === '${fund_in_method}') ? 'CASH_ON_HAND'
        : 'BANK_TRANSFER'];
    ...
    var listTable = $('#tblTransaction').dataTable({
      ...
      "columns": [
        ...
        {"data": "info"},
        {
          ...
          "render": function (data, type, full, meta) {
            var htmlCode = ""
                + "<a href=\"/fundin/${fund_in_method}/management/detail/" + data.id
                + "\">${detail_label}</a>";

            if (data.stage == 0 && data.refTxnStatus == 0) {
              htmlCode += ""
                  + "<a class=\"ml-10\" href=\"/fundin/${fund_in_method}/management/detail/"
                  + data.id
                  + "/edit\">${edit_label}</a>";
            }
